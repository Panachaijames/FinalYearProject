	...
	Score(0.f)
{
	...
	//Construct left and right collision box
...
void AEnemy::BeginPlay()
{
	...
	// Bind function to overlap event fr weapon boxes
...
void AEnemy::AggroSphereOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFormSweep, const FHitResult& SweepResult)
{
	if (OtherActor == nullptr) return;
	auto Character = Cast<ACharacterMovement>(OtherActor);
...
{
	if (OtherActor == nullptr) return;
	auto ShooterCharacter = Cast<ACharacterMovement>(OtherActor);
	if (ShooterCharacter)
	{
		bInAttackRange = true;
		if (EnemyController)
...
void AEnemy::DoDamage(ACharacterMovement* Victim)
{
...
void AEnemy::SpawnBlood(ACharacterMovement* Victim, FName SocketName)
{
...
void AEnemy::StunCharacter(ACharacterMovement* Victim)
{
	if (Victim)
	{
